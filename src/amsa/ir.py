...
class ProductIR:
    ...
    def show(self, algebra: AlgebraSpec) -> str:
        ...
        lines.append(f"  Output blades: {', '.join(algebra.blade_name(b) for b in self.out_blades)}")
        lines.append(f"  Terms ({len(self.terms)}):")
        for term in self.terms:
            coeff_str = f"{term.coefficient:+d}" if term.coefficient != 1 else "+"
            lines.append(f"    {coeff_str} col[{term.lhs_col}] * col[{term.rhs_col}] -> col[{term.out_col}]")
        return "\n".join(lines)
