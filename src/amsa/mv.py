...
class MVArray:
    ...
    def __repr__(self) -> str:
        ...
        if self.batch_shape:
            return (
                f"MVArray(batch_shape={self.batch_shape}, "
                f"blades={len(self.layout.blades)}, dtype={self.dtype})"
            )

        parts = []
        for blade, value in zip(self.layout.blades, self.values, strict=True):
